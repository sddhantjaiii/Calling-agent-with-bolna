...
-- Drop any old conflicting indexes/constraints that include 'hour'
DROP INDEX IF EXISTS idx_agent_analytics_unique_daily;
DROP INDEX IF EXISTS idx_agent_analytics_hourly;

-- Ensure the correct unique constraint exists (agent_id, user_id, date) WITHOUT hour
CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_analytics_unique_agent_user_date
ON agent_analytics(agent_id, user_id, date);

-- Recreate the trigger function with correct ON CONFLICT clause
