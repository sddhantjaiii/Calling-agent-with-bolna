-- Fix numeric overflow in user analytics CTA totals function
-- This migration fixes the DECIMAL(5,2) overflow issue in CTA conversion rate calculations

-- Update the user analytics CTA totals function to prevent numeric overflow
...
BEGIN
    ...
    -- Calculate other rates with overflow protection
    IF NEW.total_calls > 0 THEN
...
VALUES (
  'migration_027_completed_at',
  CURRENT_TIMESTAMP::TEXT,
  'Timestamp when migration 027 (fix user analytics CTA overflow) was completed',
  NULL
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE routine_name = 'update_user_analytics_cta_totals' 
        AND routine_type = 'FUNCTION'
    ) THEN
        RAISE NOTICE 'Migration 027 completed successfully: user_analytics_cta_totals function updated with overflow protection';
    ELSE
        RAISE EXCEPTION 'Migration 027 failed: update_user_analytics_cta_totals function not found';
    END IF;
END $$;